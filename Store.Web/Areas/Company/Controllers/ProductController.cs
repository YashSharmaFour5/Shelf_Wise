...
namespace Store.Web.Areas.Company.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Upsert(CompanyProduct model)
        {
            ...
            model.CompanyId = user.CompanyId.Value; // 👈 Set it explicitly here
            ...
            if (ModelState.IsValid)
            {
                var cp = await unitOfWork.CompanyProduct.GetFirstOrDefault(
                    r => r.CompanyId == model.CompanyId && r.ProductId == model.ProductId);
        ...
        public async Task<IActionResult> GetAllCompanyProducts()
        {
            ...
            */
            var user = await userManager.GetUserAsync(User) as ApplicationUser;
            if (user == null || user.CompanyId == null)
                return Json(new { data = new List<CompanyProduct>() });

            var allCompanyProducts = await unitOfWork.CompanyProduct.GetAll(includeProperties: "Product");
            var filtered = allCompanyProducts.Where(cp => cp.CompanyId == user.CompanyId).ToList();
        ...
        public async Task<IActionResult> GetAllProducts()
        {
            ...
            */
            var user = await userManager.GetUserAsync(User) as ApplicationUser;
            if (user == null || user.CompanyId == null)
                return Json(new { data = new List<Product>() }); // Always return a 'data' property

            var products = await unitOfWork.Product.GetAll(includeProperties: "Category");
            var companyProducts = (await unitOfWork.CompanyProduct.GetAll())
                                    .Where(cp => cp.CompanyId == user.CompanyId);

            var exclusiveProducts = products
                                    .Where(p => !companyProducts.Any(cp => cp.ProductId == p.Id))
                                    .Select(p => new {
                                        p.Id,
                                        p.Title,
                                        p.Author,
